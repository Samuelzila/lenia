...
//Paramètres pour les noyaux
int R = 13*8; // Le rayon du noyau de convolution actuelle
static final int R1 = 13*8; // Le rayon de convolution du premier noyau
static final int R2 = 10*8; // Le rayon pour le deuxième noyau de convolution
...
float[] preCalculateKernel(float[] beta, int[] kernelTemp) {
...
void runAutomaton(float dt) {
  ...
  kernel = preCalculateKernel(BETA, kernel2);
  convolve();
  ...
  for(int i = 0; i < potential.length; i++) {
    potential[i] = 0.5*(potential1[i]+potential2[i]);
  }
...
float[] getPolarRadiusMatrix(int[] kernel) {
  float[] matrix = new float[kernel[1]*kernel[1]];
  for (int x = -kernel[0]; x <= kernel[0]; x++)
    for (int y = -kernel[0]; y <= kernel[0]; y++)
      matrix[(x + kernel[0]) * kernel[1] + y + kernel[0]] = sqrt(x*x + y*y) * dx;
